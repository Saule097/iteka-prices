...
def parse_drug(name, slug, city):
    ...
    try:
        ...
        if table:
            ...
            # медиана из min и max
            lo = row["Мин. цена (тг)"]
            hi = row["Макс. цена (тг)"]
            if lo and hi:
                row["Медиана цены (тг)"] = round((lo + hi) / 2, 2)
        else:
            print(f"  нет таблицы price-statistic: {name}")
...
# ══════════════════════════════════════════════════
# СОХРАНЕНИЕ В GOOGLE SHEETS — пакетом (1 запрос)
# ══════════════════════════════════════════════════
...
def main():
    ...
    for name, slug in DRUGS.items():
        ...
        # безопасный вывод — проверяем None перед форматированием
        min_p  = row["Мин. цена (тг)"]
        avg_p  = row["Средняя цена (тг)"]
        max_p  = row["Макс. цена (тг)"]
        mode_p = row["Чаще всего продают по (тг)"]
        aptек  = row["Аптек"]

        if avg_p is not None:
            print(
                f"Мин: {min_p:,.0f} | Ср: {avg_p:,.0f} | "
                f"Макс: {max_p:,.0f} | "
                f"Мода: {mode_p:,.0f} | Аптек: {aptек}"
            )
        else:
            print("нет данных")
